...
{
	public class FakeQuery<T> : IQuery<T>, IEnumerable where T : class
	{
		private IQueryable<T> _query;

		public FakeQuery(IQueryable<T> query)
		{
			_query = query;
		}
		...
		{
			return new FakeQuery<TResult>(_query.Select(selector));
		}

		public IQuery<IGrouping<TKey, T>> GroupBy<TKey>(Expression<Func<T, TKey>> groupBy)
		{
			return new FakeQuery<IGrouping<TKey, T>>(_query.GroupBy(groupBy));
		}
		...
		{
			return new FakeQuery<IGrouping<TKey, TValue>>(_query.GroupBy(keySelector, valueSelector));
		}
		...
		{
			var innerQuery = (inner as FakeQuery<TInner>)?._query;
			if (innerQuery == null) throw new ArgumentException("Problem in GroupJoin: inner as EntityFrameworkQuery<TInner>");

			return new FakeQuery<TResult>(_query.GroupJoin(innerQuery, outerKeySelector, innerKeySelector, resultSelector));
		}
			...
			Expression<Func<TInner, TKey>> innerKeySelector, Expression<Func<T, TInner, TResult>> resultSelector) where TInner : class where TResult : class
		{
			var innerQuery = (inner as FakeQuery<TInner>)?._query;
			if (innerQuery == null)
				return new FakeQuery<TResult>(_query.Join(inner, outerKeySelector, innerKeySelector, resultSelector));

			return new FakeQuery<TResult>(_query.Join(innerQuery, outerKeySelector, innerKeySelector, resultSelector));
		}
		...
		{
			return new FakeQuery<TResult>(_query.SelectMany(collectionSelector, resultSelector));
		}
		...
		{
			var innerQuery = (inner as FakeQuery<T>)?._query;
			if (innerQuery == null) throw new ArgumentException("Problem in GroupJoin: inner as EntityFrameworkQuery<TInner>");

			return new FakeQuery<T>(_query.Union(innerQuery));
		}
	}
}
